...
root = tk.Tk()
root.withdraw()
file_path = filedialog.askopenfilename(filetypes=[("Four Column CSV","*.csv")])
print(file_path)
...
plt.plot(t, y, c='b', linewidth=0.7 )
plt.xlabel('Time (seconds)')
plt.ylabel('Accel (mg)')
plt.ylim(-2500, 2500)
plt.title('Y Axis')
...
plt.plot(t, z, c='g', linewidth=0.7 )
plt.xlabel('Time (seconds)')
plt.ylabel('Accel (mg)')
plt.ylim(-2500, 2500)
plt.title('Z Axis')
...
plt.plot(t_RMS, x_RMS, linewidth=0.7)
plt.xlabel('Time (seconds)')
...
#plt.figure(4) 
plt.subplot2grid((12,1), (4,0), rowspan=3, colspan=1) 
plt.plot(t_RMS, y_RMS, linewidth=0.7)
plt.xlabel('Time (seconds)')
plt.ylabel('RMS Accel (mg)')
...
yf = fft(x)
plt.plot(xf, 2.0/N * np.abs(yf[0:np.int(N/2)]), linewidth=0.7)
plt.grid()
