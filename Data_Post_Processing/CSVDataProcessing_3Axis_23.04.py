...
root = tk.Tk()
root.withdraw()
file_path = filedialog.askopenfilename(filetypes=[("Four Column CSV","*.csv")])  # Expects CSV data format of 't, X, Y, Z'
print(file_path)
...
#Plot Y Data
tic = time.process_time()
#plt.figure(2) #plots Y data on separate figure
#plt.subplot(312) 
plt.subplot2grid((12,1), (4,0), rowspan=3, colspan=1) 
plt.plot(t, y, c='g', linewidth=0.7 )
plt.xlabel('Time (seconds)')
plt.ylabel('Accel (mg)')
plt.ylim(-2500, 2500)
plt.title('Y Axis')
...
#Plot Z Data
tic = time.process_time()
#plt.figure(3) #plots Z data on separate figure
#plt.subplot(313)
plt.subplot2grid((12,1), (8,0), rowspan=3, colspan=1)
plt.plot(t, z, c='b', linewidth=0.7 )
plt.xlabel('Time (seconds)')
plt.ylabel('Accel (mg)')
plt.ylim(-2500, 2500)
plt.title('Z Axis')
...
#Compute Y axis RMS and Plot
tic = time.process_time()
w = np.int(np.floor(Fs)); #width of the window for computing RMS
steps = np.int_(np.floor(N/w)); #Number of steps for RMS
t_RMS = np.zeros((steps,1)); #Create array for RMS time values
y_RMS = np.zeros((steps,1)); #Create array for RMS values
for i in range (0, steps):
	t_RMS[i] = np.mean(t[(i*w):((i+1)*w)]);
	y_RMS[i] = np.sqrt(np.mean(y[(i*w):((i+1)*w)]**2));  
#plt.figure(4) 
plt.subplot2grid((12,1), (4,0), rowspan=3, colspan=1) 
plt.plot(t_RMS, y_RMS, c='g', linewidth=0.7)
plt.xlabel('Time (seconds)')
plt.ylabel('RMS Accel (mg)')
plt.ylim(-2500,2500)
plt.title('RMS - ' + 'Y Axis')
plt.grid()
toc = time.process_time()
print("Y-RMS Time:",toc-tic)

#Compute Z axis RMS and Plot
tic = time.process_time()
w = np.int(np.floor(Fs)); #width of the window for computing RMS
steps = np.int_(np.floor(N/w)); #Number of steps for RMS
t_RMS = np.zeros((steps,1)); #Create array for RMS time values
z_RMS = np.zeros((steps,1)); #Create array for RMS values
for i in range (0, steps):
	t_RMS[i] = np.mean(t[(i*w):((i+1)*w)]);
	z_RMS[i] = np.sqrt(np.mean(z[(i*w):((i+1)*w)]**2));  
#plt.figure(4) 
plt.subplot2grid((12,1), (8,0), rowspan=3, colspan=1) 
plt.plot(t_RMS, z_RMS, c='b', linewidth=0.7)
plt.xlabel('Time (seconds)')
plt.ylabel('RMS Accel (mg)')
plt.ylim(-2500,2500)
plt.title('RMS - ' + 'Z Axis')
plt.grid()
toc = time.process_time()
print("Z-RMS Time:",toc-tic)
...
#Compute and Plot Y axis FFT
tic = time.process_time()
#plt.figure(3) 
plt.subplot2grid((12,1), (4,0), rowspan=3, colspan=1) 
yyf = np.linspace(0.0,1.0//(2.0*T), N//2)    
yf = fft(y)
plt.plot(yyf, 2.0/N * np.abs(yf[0:np.int(N/2)]), c='g',linewidth=0.7)
plt.grid()
plt.xlabel('Frequency (Hz)')
plt.ylabel('Accel (mg)')
plt.title('FFT - ' + 'Y Axis')
toc = time.process_time()
print("Y-FFT Time:",toc-tic)

#Compute and Plot Z axis FFT
tic = time.process_time()
#plt.figure(3) 
plt.subplot2grid((12,1), (8,0), rowspan=3, colspan=1) 
zf = np.linspace(0.0,1.0//(2.0*T), N//2)    
yf = fft(x)
plt.plot(zf, 2.0/N * np.abs(yf[0:np.int(N/2)]), c='b', linewidth=0.7)
plt.grid()
plt.xlabel('Frequency (Hz)')
plt.ylabel('Accel (mg)')
